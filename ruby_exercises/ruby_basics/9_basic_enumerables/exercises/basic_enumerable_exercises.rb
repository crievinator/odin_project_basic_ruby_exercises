...
def display_guess_order(guesses)
  ...
  guesses.each.with_index(1) do |value, idx|
    puts "Guess ##{idx} is #{value}"
  end
end
...
def find_longest_word(word_list)
  ...
  word_list.reduce do |accum, word| 
...
word_list.reduce([]) do |memo, word|
  word.length > base_word.length   ?  memo << word : memo
end
end
...
def find_word_lengths(word_list)
  ...
  word_list.reduce(Hash.new) do |memo, word|
